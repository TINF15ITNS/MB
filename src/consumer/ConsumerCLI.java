...
/**
 * A command line interface that interacts with all kinds of classes
 * implementing the interface ConsumerIF.
 * 
		...
		while (!exit) {
			...
			if (!user.isRegistered()) {
				...
				case 3: //Subscribe to producers
					...
					String[] failedSubscriptions = user.subscribeToProducers(scanner.nextLine().trim().replaceAll("[ ]*,[ ]*+", ",").split(",")); //Regex replaces commas and all whitespaces next to them with just a comma
				...
				case 4: //Unsubscribe from producers
					...
					String[] failedUnsubscriptions = user.unsubscribeFromProducers(scanner.nextLine().trim().replaceAll("[ ]*,[ ]*+", ",").split(",")); //Regex replaces commas and all whitespaces next to them with just a comma
